...
const createPotMaterial = (profile) => {
  ...
  const material = new THREE.MeshStandardMaterial({
    ...
    roughness: 0.7,
    metalness: 0.0,
    side: THREE.DoubleSide, // This fixes the backface culling issue
  });
...
mesh.position.y = 0; // Position the pot lower to appear higher in viewport
...
camera.position.set(0, 120, 380); // Positioned higher to center pot in viewport
...
// Update the return statement of the CeramicPot3D component
return (
  <div className="flex flex-row h-screen bg-gray-100">
    {/* Left column - Controls and Dimensions */}
    <div className="w-1/5 flex flex-col border-r border-gray-200 p-1">
      <h1 className="text-md font-bold mb-1">Pot Generator</h1>
      
      {/* Control buttons at the top */}
      <div className="flex flex-col space-y-1 mb-1">
        <button
          className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-1 px-1 rounded text-xs"
        ...
        <button
          className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-1 px-1 rounded text-xs"
      ...
      {/* Pot specifications now below the buttons */}
      <div className="bg-white p-1 rounded-lg shadow-sm mb-1 text-xs">
        ...
        <ul className="text-xs space-y-0 mt-1">
          ...
          <li>Clay: {potProfile.clay}</li>
          <li>Texture: {potProfile.texture}</li>
          <li>Style: {['Wide Belly', 'Bowl', 'Cylinder', 'Amphora', 'Bottle', 'Flared Top'][potProfile.potStyle || 0]}</li>
          {potProfile.hasDivot && <li>Features: Divot/Waist</li>}
        </ul>
      </div>
    </div>
    
    {/* Right column - 3D Pot Visualization - now wider */}
